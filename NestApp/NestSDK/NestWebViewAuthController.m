...
- (void)loadView
{
    const CGFloat NavBarHeight = 64.;
    
    self.view = [[UIView alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
    [self.view setBackgroundColor:[UIColor whiteColor]];
    
    // Add a navbar to the top
    UINavigationBar *navBar = [[UINavigationBar alloc] initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width, NavBarHeight)];
    ...
    UINavigationItem *navItem = [[UINavigationItem alloc] initWithTitle:[Texts connectWithNest]];
    navItem.leftBarButtonItem = bbi;
    [navBar pushNavigationItem:navItem animated:YES];
    
    // Add a uiwebview to take up the entire view (beneath the nav bar)
    self.webView = [[UIWebView alloc] initWithFrame:CGRectMake(0, NavBarHeight, self.view.bounds.size.width, self.view.frame.size.height - NavBarHeight)];
...
- (void)viewDidLoad
{
    ...
    // Set the title
    self.title = [Texts connectWithNest];
